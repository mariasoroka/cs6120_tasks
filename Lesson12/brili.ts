...
import { writeTextFileSync } from "jsr:@std/fs/unstable-write-text-file";
 ...
 */
function evalCall(instr: bril.Operation, state: State, tracing_general: Boolean, tracing_current: Boolean, function_to_trace: String): Action {
  ...
  let retVal = evalFunc(func, newState, tracing_general, tracing_current, function_to_trace);
 ...
 */
function evalInstr(instr: bril.Instruction, state: State, tracing_general: Boolean, tracing_current: Boolean, function_to_trace: String): Action {
  // if (tracing_current) {
  ...
  if (tracing_current) {
    if (instr.op == 'br') {
      ...
      if (cond) {
        let new_instr = '{"args":["' + instr.args[0] + '"], "op":"guard","labels":["default"]}'
        // console.log(new_instr);
        writeTextFileSync('trace.txt', new_instr + "\n", { append: true });
      }
      else {
        let new_instr = '{"args":["' + instr.args[0] + '"], "op":"not","dest":"' + instr.args[0] + '"}'
        writeTextFileSync('trace.txt', new_instr + "\n", { append: true });
        new_instr = '{"args":["' + instr.args[0] + '"], "op":"guard","labels":["default"]}'
        // console.log(new_instr);
        writeTextFileSync('trace.txt', new_instr + "\n", { append: true });
        new_instr = '{"args":["' + instr.args[0] + '"], "op":"not","dest":"' + instr.args[0] + '"}'
        writeTextFileSync('trace.txt', new_instr + "\n", { append: true });
      }
    }
             ...
             instr.op != 'ret') {
      // console.log(JSON.stringify(instr));
      writeTextFileSync('trace.txt', JSON.stringify(instr) + "\n", { append: true });
    }
  ...
  case "call": {
    return evalCall(instr, state, tracing_general, tracing_current, function_to_trace);
  }
...
function evalFunc(func: bril.Function, state: State, tracing_general: Boolean, tracing_current: Boolean, function_to_trace: String): Value | null {
  // console.log(`Executing function ${func.name}`);
  let trace_current = ((func.name === function_to_trace) && tracing_general);
  // console.log(`Tracing function ${func.name}: ${trace_instr}`);
  // console.log(`${function_to_trace}`);
  // console.log(`${tracing}`);
  ...
  for (let i = 0; i < func.instrs.length; ++i) {
    ...
    if ('op' in line) {
      ...
      let action = evalInstr(line, state, tracing_general, trace_current, function_to_trace);
...
function evalProg(prog: bril.Program) {
  ...
  let profiling = false;
  let tracing = false;
  let tracing_main = false;
  let function_to_trace = '';
  ...
  let fidx = args.indexOf('-f');
  if (fidx > -1) {
    function_to_trace = args[fidx+1];
    args.splice(fidx, 2);
  }
  if (function_to_trace === 'main') {
    tracing_main = true;
  }

  if (tracing) {
    writeTextFileSync('trace.txt', '', { append: false });
  }
  ...
  evalFunc(main, state, tracing, tracing_main, function_to_trace);
